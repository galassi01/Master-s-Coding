...
%       across the two calls) x ONCE PER ENTRY OF reference_scheme_list
%       (default: no re-referencing, common-average-reference, and
%       single-electrode reference against electrode 12 and electrode
%       52 -- see reference_scheme_list below), writing one .mat file
%       per (pair, Muscle, reference scheme) combination
%       (trimmed_<imu_file>_Muscle<N>_..._<ref_tag>.mat) plus a single
%       Batch_Data_Prep_Log.csv summarizing every run, including which
%       reference scheme/electrode each row used.
%       One (pair, Muscle, reference scheme) combination failing (error,
%       sync-validation skip, or missing direction) does NOT stop the
%       batch -- it's logged and the loop moves on.
% ========================================================================

clear; clc;

% partic = [1,2,3,4,5,6,7,9,10];
%partic = [2,3,4,5,6,7,9,10];
partic = [10];
%% ------------------------- CONFIG --------------------------------------
for j = partic
...
% --- EMG RAW RE-REFERENCING SCHEMES (applied in the prep function's
%     Section 4a, to the raw EMG, BEFORE the >50 Hz high-pass and all
%     other filtering) ---
% Every entry in reference_scheme_list is run for every (pair, Muscle)
% combination, same as muscle_list above -- so by default this triples
% the number of runs (none / CAR / single-electrode-ref), each writing
% its own .mat file (the prep function appends _CAR / _ref<N> to the
% filename so these don't overwrite each other -- see that file's
% Section 4a / output-filename comments).
%
% Each entry is a struct with:
%   .method     -- 'none', 'car', or 'singleref'
%   .ref_elec   -- local electrode # (1-64), only used when method=='singleref'
%
% Default below runs all three requested options: no re-referencing,
% common-average-reference (per muscle, i.e. averaged across just that
% muscle's own 64 channels), and single-electrode reference against
% electrode 12, then again against electrode 52. Comment out entries to
% skip them (e.g. leave just 'none' + one singleref entry while you're
% still deciding which reference electrode to use).
reference_scheme_list = struct('method', {}, 'ref_elec', {});
reference_scheme_list(end+1) = struct('method', 'none',      'ref_elec', []);
if ismember(participant_num, [7, 9, 10])
    reference_scheme_list(end+1) = struct('method', 'car', 'ref_elec', []);
end
% reference_scheme_list(end+1) = struct('method', 'car',       'ref_elec', []);
% reference_scheme_list(end+1) = struct('method', 'singleref', 'ref_elec', 12);
% reference_scheme_list(end+1) = struct('method', 'singleref', 'ref_elec', 52);
...
n_refs      = numel(reference_scheme_list);
n_runs      = n_pairs * n_muscles * n_refs;
fprintf('Will run %d muscle setting(s) x %d reference scheme(s) per pair -> %d total run(s).\n', ...
    n_muscles, n_refs, n_runs);
...
log_RefMethod   = strings(n_runs, 1);
log_RefElectrode = nan(n_runs, 1);
...
for k = 1:n_pairs
    ...
    for m = 1:n_muscles
        ...
        for r = 1:n_refs
            this_ref_method = reference_scheme_list(r).method;
            this_ref_elec   = reference_scheme_list(r).ref_elec;   % [] unless method=='singleref'
            run_i = run_i + 1;

            if strcmp(this_ref_method, 'singleref')
                ref_tag_str = sprintf('%s (electrode %d)', this_ref_method, this_ref_elec);
            else
                ref_tag_str = this_ref_method;
            end

            fprintf('\n=== [%d/%d] Session %d, Rec %d  <->  %s  (Muscle %d, direction "%s", %s, %s, ref: %s) ===\n', ...
                run_i, n_runs, sess_idx, local_rec, imu_file, this_muscle, this_direction, this_paramset, this_condition, ref_tag_str);
            ...
            log_RefMethod(run_i)   = string(this_ref_method);
            if strcmp(this_ref_method, 'singleref')
                log_RefElectrode(run_i) = this_ref_elec;
            end
            ...
            % --- EMG raw re-referencing (Section 4a of the prep function,
            %     applied to the raw EMG BEFORE any filtering) ---
            P.emg_reference_method  = this_ref_method;   % 'none' / 'car' / 'singleref'
            if strcmp(this_ref_method, 'singleref')
                P.emg_ref_electrode = this_ref_elec;     % local electrode # (1-64), e.g. 12 or 52
            end
            ...
            try
                ...
                data = KUKA_analysis_prep_consume_ref(P);
...
batch_log = table(log_ImuFile, log_SessionIdx, log_LocalRec, log_Muscle, ...
    log_Direction, log_RefMethod, log_RefElectrode, log_Status, log_Message, log_MatPath, log_ElapsedSec, ...
    'VariableNames', {'IMU_FileName', 'EMG_SessionIdx', 'EMG_LocalRec', ...
    'Muscle', 'Direction', 'RefMethod', 'RefElectrode', 'Status', 'Message', 'MatPath', 'ElapsedSec'});
...
fprintf(['\n=== BATCH COMPLETE: %d ok, %d skipped (sync fail), ' ...
    '%d skipped (no direction), %d error, out of %d run(s) [%d pair(s) x %d muscle setting(s) x %d reference scheme(s)] ===\n'], ...
    n_ok, n_skipped_sync, n_skipped_direction, n_error, n_runs, n_pairs, n_muscles, n_refs);
