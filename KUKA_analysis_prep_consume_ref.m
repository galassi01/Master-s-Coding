...
function data = KUKA_analysis_prep_consume_ref(P) % sept 12 2026 version
...
%% 4a. RAW EMG RE-REFERENCING (BEFORE ANY FILTERING)
% Applied here, to raw_emg_array, before the >50 Hz high-pass -- and
% therefore before every downstream derivative (rectify, RMS,
% mu-centered bandpass, cycle-trim, etc.), all of which are built off
% the high-pass signal. Selectable via P.emg_reference_method:
%
%   'none'      -- (default) no re-referencing; raw_emg_array unchanged.
%
%   'car'       -- Common Average Reference. For THIS muscle only (i.e.
%                  averaged across just the 64 channels in ch_range --
%                  1:64 for Muscle==1, 65:128 for Muscle==2 -- never
%                  across both muscles at once), compute the mean signal
%                  across electrodes at each sample, then subtract that
%                  single across-electrode-average trace from every
%                  channel in this muscle's block.
%
%   'singleref' -- Single-electrode reference. Subtract one specific
%                  electrode's raw signal from every channel in this
%                  muscle's ch_range (including itself, which becomes an
%                  all-zero row by design -- useful as a sanity check).
%                  P.emg_ref_electrode is a LOCAL electrode number
%                  (1-64) within THIS muscle's grid -- e.g. 12 or 52 --
%                  not a global 1-128 channel index, so the same
%                  physical grid position is used regardless of which
%                  muscle (Muscle==1 or Muscle==2) is being processed.
%
% Only one scheme runs per call. To compare methods (e.g. CAR vs.
% electrode 12 vs. electrode 52), call this function once per method/
% electrode you want, and compare the resulting data.emg.reference_*
% metadata + outputs across runs -- schemes are not stacked/combined.
emg_reference_method = 'none';
if isfield(P, 'emg_reference_method') && ~isempty(P.emg_reference_method)
    emg_reference_method = lower(P.emg_reference_method);
end
emg_ref_electrode_used = NaN;   % local (1-64) electrode number actually used, if any; recorded in data.emg below

switch emg_reference_method
    case 'none'
        % raw_emg_array left untouched

    case 'car'
        car_signal    = mean(raw_emg_array, 1, 'omitnan');   % 1 x nSamples, this muscle's across-electrode average
        raw_emg_array = raw_emg_array - car_signal;          % broadcast-subtract from every channel in ch_range

    case 'singleref'
        if ~isfield(P, 'emg_ref_electrode') || isempty(P.emg_ref_electrode)
            error(['P.emg_reference_method = ''singleref'' requires P.emg_ref_electrode ' ...
                   '(a local electrode number 1-%d within this muscle''s grid, e.g. 12 or 52).'], n_emg_channels);
        end
        emg_ref_electrode_used = P.emg_ref_electrode;
        if emg_ref_electrode_used < 1 || emg_ref_electrode_used > n_emg_channels || ...
                emg_ref_electrode_used ~= round(emg_ref_electrode_used)
            error(['P.emg_ref_electrode (%g) must be an integer between 1 and %d ' ...
                   '(this muscle''s local electrode range).'], emg_ref_electrode_used, n_emg_channels);
        end
        ref_signal    = raw_emg_array(emg_ref_electrode_used, :);   % 1 x nSamples
        raw_emg_array = raw_emg_array - ref_signal;                  % broadcast-subtract from every channel in ch_range (incl. the reference itself -> zero row)

    otherwise
        error('Unrecognized P.emg_reference_method ''%s''. Must be ''none'', ''car'', or ''singleref''.', emg_reference_method);
end

[b_hp, a_hp] = butter(2, 50 / (emg_fs/2), 'high');
%[b_hp, a_hp] = butter(2, 100 / (emg_fs/2), 'high');
...
data.emg.reference_method       = emg_reference_method;  % 'none' / 'car' / 'singleref' -- re-referencing applied to raw_emg_array in Section 4a, BEFORE the >50 Hz high-pass and all downstream filtering
data.emg.reference_electrode    = emg_ref_electrode_used; % local (1-64) electrode number used when reference_method=='singleref' (e.g. 12 or 52); NaN otherwise
...
data.meta = build_meta(direction, mu, T_cycle, num_cycles, idxStart_emg, idxEnd_emg, ...
    idxStart_imu, idxEnd_imu, num_imus_detected, imu_file, rec_num, movement_buffer_sec, 'trial_data');
data.meta.dur_sync_diff_pct   = dur_diff_pct;
data.meta.dur_cycle_diff_pct  = dur_cycle_diff_pct;
...
% explicitly so cycle-trim accuracy can be checked directly against the
% sync-trimmed arrays without re-deriving this relative/absolute
% conversion. Both the IMU and EMG naming collisions that used to affect
% this block are fixed -- build_meta's parameters are now named
% idxStart_imu_synctrim_abs/idxEnd_imu_synctrim_abs and
% idxStart_emg_synctrim_abs/idxEnd_emg_synctrim_abs and carry the true
% sync-trim boundaries (see data.meta.idxStart_imu_synctrim_abs /
% idxStart_emg_synctrim_abs below), distinct from the *_cycle_rel fields
% here.
...
% Filename: trimmed_<imu_base>_Muscle<N>_<direction>_<ParameterSet>_<condition>[_<ref_tag>].mat
% e.g. trimmed_KUKA_neckEMG_P06_Trial_Block1_01_Muscle1_AP_1800_0504_EO.mat           (reference_method == 'none')
%      trimmed_KUKA_neckEMG_P06_Trial_Block1_01_Muscle1_AP_1800_0504_EO_CAR.mat       (reference_method == 'car')
%      trimmed_KUKA_neckEMG_P06_Trial_Block1_01_Muscle1_AP_1800_0504_EO_ref52.mat     (reference_method == 'singleref', electrode 52)
% The ref_tag suffix is appended ONLY when a re-referencing scheme is
% actually used, so 'none' output filenames are unchanged from before --
% this exists so that running the same pair/muscle through 'none', 'car',
% and 'singleref' (electrode 12, then 52) for comparison writes four
% distinct .mat files instead of the later runs silently overwriting the
% earlier ones.
switch emg_reference_method
    case 'car'
        ref_tag = '_CAR';
    case 'singleref'
        ref_tag = sprintf('_ref%d', emg_ref_electrode_used);
    otherwise
        ref_tag = '';
end
data_mat_path = fullfile(csv_out_dir, sprintf('trimmed_%s_Muscle%d_%s_%s_%s%s.mat', ...
    imu_fname_tag, Muscle, direction, ParameterSet, condition, ref_tag));
data.meta.mat_path = data_mat_path;   % so callers/batch driver know where this pair's file landed
save(data_mat_path, 'data', '-v7.3');
fprintf('Saved Level 3 (trial data, cycle-trimmed) data (%d EMG channels, flange + head IMU) to:\n  %s\n\n', ...
    n_emg_channels, data_mat_path);
...
function meta = build_meta(direction, mu, T_cycle, num_cycles, idxStart_emg_synctrim_abs, idxEnd_emg_synctrim_abs, ...
    idxStart_imu_synctrim_abs, idxEnd_imu_synctrim_abs, num_imus_detected, imu_file, rec_num, movement_buffer_sec, level_name)
    meta = struct();
    ...
    meta.idxStart_emg_synctrim_abs = idxStart_emg_synctrim_abs;   meta.idxEnd_emg_synctrim_abs = idxEnd_emg_synctrim_abs;
    meta.idxStart_imu_synctrim_abs = idxStart_imu_synctrim_abs;   meta.idxEnd_imu_synctrim_abs = idxEnd_imu_synctrim_abs;
